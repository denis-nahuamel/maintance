...
public codIncidente:string;
  ...
  ngOnInit(): void {
    this.codIncidente = this.activatedRoute.snapshot.paramMap.get('codIncidente');//idComponente
    if(this.codIncidente){
      this.titulo="Modificar Componente";
      this.incidenciaService.getIncidencia(this.codIncidente).subscribe(incidente => {             
 ...
 agregarIncidencia(incidente:IncidenteInterface){
  	 if(this.codIncidente){//actualizar catalogo
       this.incidenciaService.actualizarIncidencia(this.codIncidente,incidente).subscribe(res => {
